...
func SelectProblemByStatusAndId(id uint64, s model.ProblemStatus) (model.Problem, error) {
	...
	err := db.Mysql.QueryRow(sql, s, id).Scan(&p.Title, &p.Source, &p.Difficulty, &p.TimeLimit, &p.MemoryLimit, &p.Description, &p.Input, &p.Output, &p.SampleInput, &p.SampleOutput, &p.Hint, &p.Status, &createTimeStr, &updateTimeStr)
...
func SelectProblemsByStatusAndTagId(tid uint64, s model.ProblemStatus) ([]model.Problem, error) {
	...
	rows, err := db.Mysql.Query(sql, s, tid)
...
func SelectProblemsByStatusAndDifficulty(d model.ProblemDifficulty, s model.ProblemStatus) ([]model.Problem, error) {
	...
	rows, err := db.Mysql.Query(sql, s, d)
	log.Println(sql, s, d)
	if err != nil {
		return nil, err
	}
