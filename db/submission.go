...
func SelectSubmissionById(id uint64) (model.Submission, error) {
	...
	err := SqlDb.QueryRow(sql, id).Scan(&s.UserId, &s.ProblemId, &s.Status, &s.Score, &s.LanguageId, &s.Length, &s.Memory, &s.Time, &s.SourceCode, &createTimeStr, &updateTimeStr)
	log.Println(sql, id)
	if err != nil {
...
func SelectSubmissionsByUserId(user_id uint64) ([]model.Submission, error) {
	...
	rows, err := SqlDb.Query(sql, user_id)
...
func SelectSubmissionsByProblemId(problem_id uint64) ([]model.Submission, error) {
	...
	rows, err := SqlDb.Query(sql, problem_id)
